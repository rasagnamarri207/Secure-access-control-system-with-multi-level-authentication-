...
#include"types.h"
#include"i2c.h"
void i2c_eeprom_write_byte(u8 slaveAddr,u8 wBuffAddr,u8 dat)
{
...
u8 i2c_eeprom_read_byte(u8 slaveAddr,u8 rBuffAddr)
{
	u8 dat;
...
void i2c_eeprom_write_page(u8 slaveAddr,u8 wBuffStartAddr,s8 *p,u8 nBytes)
{
	int i;
	i2c_start();
	i2c_write(slaveAddr<<1);
...
void i2c_eeprom_seq_read(u8 slaveAddr,u8 rBuffStartAddr, s8 *p,u8 nBytes)
{
	u8 i;
	i2c_start();
	i2c_write(slaveAddr<<1);
